...
const Personal = ({userId}) =>{
    ...
    React.useEffect(()=>{
        async function getUser(){
            let req = await fetch(`http://localhost:8080/user/${userId}`) ; 
            if(req.status!=200)  ; 
            let res = await req.json() ; 
            console.log(res) ; 
            setEmail(res.email) ; 
            let fullName = res.fullName.split(" ") ; 
    ...
    const save = async () =>{
        ...
        const options = {
          ...
          let req = await fetch(`http://localhost:8080/user?userId=${userId}`,options) ;
          if(req.status==200) {
            setSuccess(true) ; 
  ...
  <Stack spacing={1}>
    ...
    <FormControl>
      ...
      />
        ...
        { success && <Alert severity="success">Profile has been updated</Alert> }
